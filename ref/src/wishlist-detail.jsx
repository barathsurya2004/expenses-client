...
const { useState: wdUseState, useMemo: wdUseMemo } = React;

function WishlistDetail({ item, state, onBack, onEdit, onAdjustAllocation, onAllocate }) {
  if (!item) return null;
  ...
  // Total savings pool (sum of all wishlist allocations in state)
  ...
  return (
    <div className="phone-scroll fade-up" style={{
      ...
      <div style={{ position: 'relative', width: '100%', aspectRatio: '4 / 3', overflow: 'hidden' }}>
        ...
        {/* Gradient overlay for status bar legibility */}
        <div style={{
          position: 'absolute', top: 0, left: 0, right: 0, height: 120,
          background: 'linear-gradient(180deg, rgba(0,0,0,0.4) 0%, transparent 100%)',
          pointerEvents: 'none',
        }} />
        {/* Gradient overlay bottom for content blend */}
        <div style={{
      ...
      <div style={{ padding: '20px 16px 0' }}>
        <Card padding={22} style={{
          ...
          <div style={{
            ...
            <Lucide name={funded ? 'check' : 'clock'} size={16} stroke={2} />
        ...
        <div style={{ padding: '12px 16px 0' }}>
          <div onClick={onAllocate} className="press" style={{
            ...
            <div style={{
              ...
              <Lucide name="plus" size={18} stroke={2.6} />
            </div>
            <div style={{ flex: 1 }}>
            ...
            {state.unallocated > 0 && <Lucide name="arrow-up-right" size={18} stroke={2.2} />}
          </div>
        </div>
      )}
...
function GlassButton({ onClick, icon, label }) {
  return (
    <div onClick={onClick} className="press" style={{
      ...
      {icon && <Lucide name={icon} size={16} stroke={2.2} />}
      {label && <span style={{ fontSize: 14, fontWeight: 600, letterSpacing: -0.1 }}>{label}</span>}
    </div>
  );
}

function DetailRow({ label, value, valueNode, valueColor, icon, bold }) {
  return (
    <div style={{
      padding: '14px 18px',
...
Object.assign(window, { WishlistDetail });
