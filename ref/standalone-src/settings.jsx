// Settings — currency, profile, theme, accent

const { useState: sUseState } = React;

const CURRENCY_LIST = [
  { code: 'INR', label: 'Indian Rupee',  symbol: '₹',   sub: 'INR' },
  { code: 'USD', label: 'US Dollar',     symbol: '$',   sub: 'USD' },
  { code: 'EUR', label: 'Euro',          symbol: '€',   sub: 'EUR' },
  { code: 'GBP', label: 'British Pound', symbol: '£',   sub: 'GBP' },
  { code: 'JPY', label: 'Japanese Yen',  symbol: '¥',   sub: 'JPY' },
  { code: 'AED', label: 'UAE Dirham',    symbol: 'AED', sub: 'AED' },
  { code: 'SGD', label: 'Singapore Dollar', symbol: 'S$', sub: 'SGD' },
];
...
function SettingsScreen({ user, settings, onUpdate, onSignOut, onBack }) {
  const [editingName, setEditingName] = sUseState(false);
  const [nameDraft, setNameDraft] = sUseState(user.name || '');
  const [emailDraft, setEmailDraft] = sUseState(user.email || '');
    ...
    <div className="phone-scroll fade-up" style={{
      ...
      <div style={{
        ...
        <div onClick={onBack} className="press" style={{
          ...
          <Lucide name="chevron-left" size={18} stroke={2} />
          Back
        </div>
      </div>
          ...
          <div style={{
            ...
            {editingName ? (
              ...
              <div className="press" onClick={() => { setNameDraft(user.name); setEmailDraft(user.email || ''); setEditingName(true); }} style={{
                ...
                <Lucide name="pencil" size={14} stroke={2} />
                Edit
              </div>
            )}
          </div>
            ...
            {CURRENCY_LIST.map((c, i) => {
              ...
              return (
                <div key={c.code}
                  ...
                  {active && <Lucide name="check" size={14} stroke={3} style={{ color: 'var(--copper)' }} />}
                </div>
              );
            })}
          </div>
            ...
            {ACCENT_OPTIONS.map(a => {
              ...
              return (
                <div key={a.value} onClick={() => onUpdate({ accent: a.value })} className="press"
                  ...
                  {active && <Lucide name="check" size={18} stroke={3} style={{ color: 'white' }} />}
                </div>
              );
            })}
          </div>
      ...
      <SettingsSection title="Other">
        <Card padding={0}>
          ...
          <Hr />
          <Row label="Reset to defaults" tone="copper" trailing={<Lucide name="chevron-right" size={16} style={{ color: 'var(--ink-4)' }} />}
            onClick={() => onUpdate({ currency: 'INR', theme: 'light', accent: '#C5703B', palette: 'sand' })} />
          <Hr />
          <Row label="Sign out" tone="clay" trailing={<Lucide name="chevron-right" size={16} style={{ color: 'var(--ink-4)' }} />}
            onClick={onSignOut} />
        </Card>
      </SettingsSection>
    ...
    <div onClick={onClick} className="press" style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <Lucide name={isLight ? 'sun' : 'moon'} size={16} stroke={2} />
        </div>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)', textTransform: 'capitalize' }}>{mode}</div>
        {active && <Lucide name="check" size={16} stroke={3} style={{ color: 'var(--copper)' }} />}
      </div>
    </div>
  );
}

function PreviewRow({ currency }) {
  // Show sample numbers in the currently selected currency by temporarily switching context.
  const samples = [125, 12500, 1450000];
  const prev = window.__APP_CURRENCY;
  window.__APP_CURRENCY = currency;
  const out = samples.map(n => fmtINR(n));
  window.__APP_CURRENCY = prev;
...
function Row({ label, trailing, onClick, tone }) {
  const toneMap = {
    copper: 'var(--copper)',
    clay: 'var(--clay)',
  };
...
Object.assign(window, { SettingsScreen, PALETTE_OPTIONS });
