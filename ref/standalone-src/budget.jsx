// Budget screen — allocation matrix

function Budget({ state, onEditCategory, onAddCategory, onTogglePaid }) {
  const { budget } = state;

  const tierStats = ['needs', 'savings', 'wants'].map(k => {
  ...
  // 50/30/20 actuals
  const splitPct = {
    needs: tierStats[0].limit / totalLimit,
    savings: tierStats[1].limit / totalLimit,
    wants: tierStats[2].limit / totalLimit,
  };

  const fixedCostRatio = tierStats[0].limit / budget.income;
  const adherence = totalSpent <= totalLimit ? 100 - Math.round((totalSpent / totalLimit) * 100 - 50) * 2 : Math.round(100 - (totalSpent - totalLimit) / totalLimit * 100);
...
function Slice({ pct, color, label }) {
...
function Legend({ color, label, actual, target }) {
...
function TierBlock({ tier, idx, onEditCategory, onAddCategory, onTogglePaid }) {
  const tierLabel = { 1: 'Tier I', 2: 'Tier II', 3: 'Tier III' }[idx];
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span className="press" onClick={onAddCategory} style={{
            ...
            <Lucide name="plus" size={13} stroke={2.4} />
            New
          </span>
        </div>
      </div>
...
function CategoryRow({ cat, onEdit, isLast }) {
  const pct = cat.limit > 0 ? cat.spent / cat.limit : 0;
  const over = pct > 1;
  const status = pct > 1 ? 'clay' : pct > 0.85 ? 'amber' : 'green';
    ...
    <div onClick={onEdit} className="press" style={{
      ...
      {over && (
        <div style={{
          ...
          <Lucide name="alert" size={11} /> {fmtINR(cat.spent - cat.limit)} over
        </div>
      )}
    </div>
  );
...
function formatFrequency(cat) {
  if (!cat.frequency) return '';
  if (cat.frequency === 'daily') return 'Every day';
  if (cat.frequency === 'weekly') return 'Every week';
  if (cat.frequency === 'monthly') return 'Every month';
  if (cat.frequency === 'custom') {
    const n = cat.customCount || 1;
    return 'Every ' + (n === 1 ? cat.customUnit?.replace(/s$/, '') : n + ' ' + cat.customUnit);
  }
  return '';
}

function BillRow({ cat, onEdit, onTogglePaid, isLast }) {
  const paid = !!cat.paid;
    ...
    <div className="press" onClick={onEdit} style={{
      ...
      <div style={{ textAlign: 'right' }}>
        ...
        <div
          ...
          <Lucide name={paid ? 'check' : 'clock'} size={10} stroke={2.6} />
          {paid ? 'Paid' : 'Unpaid'}
        </div>
      </div>
    </div>
