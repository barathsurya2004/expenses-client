...
import { Card, CategoryTile, PageHeader, BottomSheet } from '../components/Common';
import { fmtINR, relativeDay, relativeTime, dayKey, CATEGORY_LIST } from '../data';
...
export const Transactions: React.FC<TransactionsProps> = ({ state, onEditTxn, setTransactions, onDeleteTxn }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return xs.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  const bulkRecategorize = (newCat: string) => {
    ...
    <div className="phone-scroll fade-up" style={{ height: '100%', overflowY: 'auto', paddingBottom: 110 }}>
      ...
      <div style={{ padding: '0 16px 8px' }}>
        <div style={{
          ...
          <LucideIcon name="search" size={16} style={{ color: 'var(--ink-3)' }} />
          ...
          {query && (
            <div className="press" onClick={() => setQuery('')} style={{ color: 'var(--ink-3)' }}>
              <LucideIcon name="close" size={16} stroke={2} />
            </div>
          )}
        </div>
      </div>
        ...
        <div style={{ padding: '0 16px 8px' }}>
          <div style={{
            ...
            <div className="press" onClick={() => setBulkOpen(true)} style={{
              ...
              <LucideIcon name="tag" size={12} stroke={2.2} />
              Re-categorize
            </div>
            <div className="press" onClick={bulkDelete} style={{
              ...
              <LucideIcon name="trash" size={12} stroke={2.4} />
              Delete
            </div>
            <div className="press" onClick={clearSel} style={{ opacity: 0.7, marginLeft: 2 }}>
              <LucideIcon name="close" size={18} stroke={2.2} />
            </div>
          </div>
        </div>
      )}
        ...
        <Card padding={0}>
          {CATEGORY_LIST.filter(c => c !== 'Salary').map((c, i, arr) => (
            <div key={c} onClick={() => bulkRecategorize(c)} className="press" style={{
              ...
              <LucideIcon name="chevron-right" size={16} style={{ color: 'var(--ink-4)' }} />
            </div>
          ))}
        </Card>
      </BottomSheet>
...
function TxnRow({ t, isLast, onClick, onLongPress, selected, selecting }: TxnRowProps) {
  const [pressTimer, setPressTimer] = useState<number | null>(null);
  const startPress = () => {
    setPressTimer(window.setTimeout(onLongPress, 450));
  };
  const endPress = () => {
    ...
    >
      {selecting && (
        <div style={{
          ...
          {selected && <LucideIcon name="check" size={14} stroke={2.8} />}
        </div>
      )}
